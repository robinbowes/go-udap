...
its music server, playing audio — does not answer UDAP at all. The
firmware's UDAP listener exits once the device reaches the connected
state, so `discover` sees nothing even though the device is on the
network and answering pings.

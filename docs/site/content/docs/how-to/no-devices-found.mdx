...
its music server, playing audio — does not answer UDAP at all, so
`discover` sees nothing even though the device is on the network and
answering pings. Why the firmware stops responding isn't confirmed;
the behaviour itself is consistent and reproducible.

Short-press the front button to drop it into setup mode (the LED
flashes red). NVRAM is preserved, and UDAP starts responding again.

See [Setup mode vs. run mode](../concepts/setup-vs-run-mode.mdx) for
the evidence behind this and the model-coverage caveat.
